...
from src.utils.config_files_utils import get_params_values
...
def per_class_loss(criterion, logits, labels, unk_masks, n_classes):
    ...
    for class_ in range(n_classes):
        ...
        class_loss.append(
            criterion(
                logits[idx.repeat(1, 1, 1, n_classes)].reshape(-1, n_classes),  # ???
                labels[idx].reshape(-1, 1),
                unk_masks[idx].reshape(-1, 1),
            )
            .detach()
            .cpu()
            .numpy()
...
class MaskedCrossEntropyLoss(torch.nn.Module):
    ...
    def forward(self, logits, ground_truth):
        ...
        if mask is not None:
            ...
            masked_logits_flat = logits.reshape(-1, logits.size(-1))
            masked_target_flat = target.reshape(-1, 1).to(torch.int64)  # (N*H*W x 1)
        #masked_log_probs_flat = torch.nn.functional.log_softmax(masked_logits_flat)
        masked_log_probs_flat = torch.nn.functional.log_softmax(masked_logits_flat, dim=1)
...
class MaskedFocalLoss(nn.Module):
    ...
    def forward(self, logits, ground_truth):
        ...
        if mask is not None:
            ...
            target = target[mask]
            logits = logits[mask.repeat(1, logits.shape[-1])].reshape(
                -1, logits.shape[-1]
            )
...
class MaskedDiceLoss(nn.Module):
    ...
    def forward(self, logits, ground_truth):
        ...
        #target_onehot = (
            #torch.eye(logits.shape[-1])[target].to(torch.float32).cuda()
...
class MaskedDiceLoss(nn.Module):
    ...
    def forward(self, logits, ground_truth):
        ...
        # ---- Dice per class (macro Dice) ----
        dims = 0  # batch dimension after flatten
...
class MaskedTverskyLoss(nn.Module):
    def __init__(
        ...
        """
        alpha: peso dei False Positives
        beta : peso dei False Negatives
        ignore_background: se True, ignora la classe 0
        """
        super().__init__()
        self.alpha = alpha
        self.beta = beta
    ...
    def forward(self, logits, ground_truth):
        ...
        if isinstance(ground_truth, torch.Tensor):
            ...
            raise ValueError(
                "ground_truth must be (target) or (target, mask)"
            )

        B, H, W, C = logits.shape

        # --- flatten ---
        logits_flat = logits.reshape(-1, C)          # (N,C)
        ...
        # --- one-hot target ---
        target_1hot = F.one_hot(target_flat, num_classes=C).float()

        # --- softmax ---
        probs = F.softmax(logits_flat, dim=1)

        # --- opzionale: ignora background ---
        if self.ignore_background:
            probs = probs[:, 1:]
            target_1hot = target_1hot[:, 1:]

        # --- TP / FP / FN ---
        tp = (probs * target_1hot).sum(dim=0)
        fp = (probs * (1 - target_1hot)).sum(dim=0)
        fn = ((1 - probs) * target_1hot).sum(dim=0)
...
class MaskedTverskyLoss(nn.Module):
    def __init__(
        ...
        mean=True,
        ignore_background=False,
        class_weights=None,   # ← NUOVO: Tensor (C,) o None
    ):
        super().__init__()
        self.alpha = alpha
        self.beta = beta
        ...
        self.mean = mean
        self.ignore_background = ignore_background
        # Registrato come buffer: si sposta su GPU automaticamente con .to(device)
        if class_weights is not None:
            self.register_buffer(
                "class_weights",
                class_weights / class_weights.sum()  # normalizza
            )
        else:
    ...
    def forward(self, logits, ground_truth):
        ...
        if isinstance(ground_truth, torch.Tensor):
            ...
            raise ValueError("ground_truth must be (target) or (target, mask)")
        ...
        if self.mean:
            if self.class_weights is not None:
                # ← UNICA RIGA CAMBIATA rispetto all'originale
                w = self.class_weights[1:] if self.ignore_background \
